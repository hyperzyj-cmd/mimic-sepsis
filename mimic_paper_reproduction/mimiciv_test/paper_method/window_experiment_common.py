r"""
Shared MIMIC-IV onset-window experiment runner.
"""
...
PARQUET = REPO_ROOT / "build_mimic" / "mimiciv" / "output" / "mimic4_wide.parquet"
...
def preload_data(
    ...
    *,
    ...
    stat, stat_select = _static_sql(active_static_cols)

    print("  Positive stays...")
    pos_static = con.execute(
        f"""
        WITH onset AS (
            SELECT stay_id, MIN(hr) AS onset_hr_first
            FROM read_parquet('{path}')
            WHERE SepsisLabel = 1
            GROUP BY stay_id
        ),
        stay_bounds AS (
            SELECT stay_id, MAX(hr) AS max_hr
            FROM read_parquet('{path}')
            GROUP BY stay_id
        ),
        pos_ids AS (
            SELECT o.stay_id, o.onset_hr_first, b.max_hr
            FROM onset o
            JOIN stay_bounds b USING (stay_id)
            WHERE o.onset_hr_first >= {cohort_before}
              AND b.max_hr >= o.onset_hr_first + {cohort_after - 1}
        )
        SELECT s.stay_id, p.onset_hr_first, p.max_hr{stat_select}
        FROM (
            SELECT stay_id{stat_select}
            FROM read_parquet('{path}')
            WHERE stay_id IN (SELECT stay_id FROM pos_ids)
            GROUP BY stay_id
        ) s
        JOIN pos_ids p USING (stay_id)
        GROUP BY s.stay_id, p.onset_hr_first, p.max_hr
    """
    ).fetchdf()
    ...
    pos_ids_str = ",".join(str(i) for i in pos_static["stay_id"].tolist())
    pos_hourly = con.execute(
        f"""
        SELECT stay_id, {dyn_select}
        FROM read_parquet('{path}')
        WHERE stay_id IN ({pos_ids_str})
        ORDER BY stay_id, hr
    """
    ).fetchdf()
    ...
    neg_static = con.execute(
        f"""
        SELECT stay_id, MAX(hr) AS los_hr{stat_select}
        FROM read_parquet('{path}')
        GROUP BY stay_id
    ...
    neg_ids_str = ",".join(str(i) for i in neg_static["stay_id"].tolist())
    neg_hourly = con.execute(
        f"""
        SELECT stay_id, {dyn_select}
        FROM read_parquet('{path}')
        WHERE stay_id IN ({neg_ids_str})
        ORDER BY stay_id, hr
    """
    ).fetchdf()
...
def _build_negative_cohort(
    ...
    *,
    ...
    if mode == "tail":
        neg_los = dict(zip(neg_static["stay_id"], neg_static["los_hr"]))
        neg_hr["los_hr"] = neg_hr["stay_id"].map(neg_los)
    ...
    valid_neg_full = set(
        neg_cohort.groupby("stay_id").size().loc[lambda s: s == cohort_total].index
    )
    neg_cohort = neg_cohort[neg_cohort["stay_id"].isin(valid_neg_full)].copy()
    neg_static_valid = neg_static[neg_static["stay_id"].isin(valid_neg_full)].copy()
...
def extract_horizon_windows(
    ...
    pos_map = dict(zip(pos_static["stay_id"], pos_static["onset_hr_first"]))
    pos_hr = pos_hourly[pos_hourly["stay_id"].isin(pos_map)].copy()
    pos_hr["onset_hr_first"] = pos_hr["stay_id"].map(pos_map)
    ...
    valid_pos_full = set(
        pos_cohort.groupby("stay_id").size().loc[lambda s: s == cohort_total].index
    )

    pos_cohort = pos_cohort[pos_cohort["stay_id"].isin(valid_pos_full)].copy()
    ...
    pos_root = pos_static[pos_static["stay_id"].isin(valid_pos_full)].drop(
    ...
    neg_visible = neg_visible_all[
        neg_visible_all["stay_id"].isin(set(neg_root["stay_id"]))
    ].reset_index(drop=True)
...
def make_dataset(root_df, hourly_df):
    ...
    return {
        "main_table": (root_df, ["stay_id"]),
        "additional_data_tables": {"hourly": (hourly_df, ["stay_id"])},
    }
...
def export_global_explainability(
    ...
    *,
    ...
    case_df = pd.DataFrame(
        {
            "stay_id": stay_ids.reset_index(drop=True),
...
def run_split(root, hourly, y, *, n_splits: int):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(splitter.split(root, y), start=1):
        ...
        ids_tr = set(root_tr["stay_id"])
        ids_te = set(root_te["stay_id"])
        hrly_tr = hourly[hourly["stay_id"].isin(ids_tr)].reset_index(drop=True)
        hrly_te = hourly[hourly["stay_id"].isin(ids_te)].reset_index(drop=True)
...
def run_experiment(
    *,
    ...
    summary_table_name: str = "mimiciv_experiment_summary.csv",
):
